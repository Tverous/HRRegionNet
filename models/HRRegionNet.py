...
class _HRRegionNet(nn.Module):
    def __init__(self, c=48, nof_joints=17, bn_momentum=0.1):
        ...
        self.final_layers.append(nn.Sequential(
            ...
            nn.BatchNorm2d(nof_joints * 2, eps=1e-05, momentum=bn_momentum, affine=True, track_running_stats=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(nof_joints * 2, nof_joints * 2, kernel_size=(1, 1), stride=(1, 1)),
            nn.Sigmoid()
        ))
        for i in range(self.num_deconvs):
            self.final_layers.append(nn.Sequential(
                ...
                nn.BatchNorm2d(nof_joints * 2, eps=1e-05, momentum=bn_momentum, affine=True, track_running_stats=True),
                nn.ReLU(inplace=True),
                nn.Conv2d(nof_joints * 2, nof_joints, kernel_size=(1, 1), stride=(1, 1)),
                nn.Sigmoid()
            ))
        ...
        for i in range(self.num_deconvs):
            ...
            deconv_kernel, padding, output_padding = 3, 1, 1

            layers = []
            layers.append(nn.Sequential(
                nn.Conv2d(input_channels, input_channels, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
                nn.BatchNorm2d(input_channels, eps=1e-05, momentum=bn_momentum, affine=True, track_running_stats=True),
                nn.ReLU(inplace=True),
                ...
                nn.BatchNorm2d(output_channels, eps=1e-05, momentum=bn_momentum, affine=True, track_running_stats=True),
                nn.ReLU(inplace=True),
            ))
            for _ in range(4):
                layers.append(nn.Sequential(
